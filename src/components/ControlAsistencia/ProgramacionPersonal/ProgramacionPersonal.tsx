...
export const ProgramacionPersonal = () => {
  ...
  const [unidadArea, setUnidadArea] = useState<any[]>([]);
  const [optionUnidad, setoptionUnidad] = useState<any[]>([]);
  const [optionArea, setoptionArea] = useState<any[]>([]);

  const [datosModal, setdatosModal] = useState<any>();
  const [optionTurnos, setoptionTurnos] = useState<any[]>([]);
  ...
  const getEmpleadosByNom = async (nom: string) => {
    try {
      const response = await getData(`${process.env.apijimmynew}/empleados/apiempleadobynomape/${nom}`);
  ...
  const getDataUnidadAreas = async () => {
    const data = await getData(`${process.env.apijimmynew}/controlasistenciaunidad/`);
    setUnidadArea(data)

    const turnos=await getData(`${process.env.apijimmynew}/controlasistenciaunidad/turnos`);

    const mappedTurnos=turnos.map((est:any)=>({
      value: est?.id,
     label: `${est?.nombre} (${est?.horaInicio?.slice(0, 5)}-${est?.horaFin?.slice(0, 5)})`
    }))
    setoptionTurnos(mappedTurnos)

    const mappedOptions = data.map((est: any) => ({
      value: est.idUnidad,
      label: est.nombre
    }));
    setoptionUnidad(mappedOptions);
  }
  ...
  const idunidadw = watch('idunidad');
  useEffect(() => {
    if (idunidadw) {
      getAreas(idunidadw)
    }
  ...
  const getAreas = async (idunidadw: any) => {
    const unidadAreaFiltrado = unidadArea.filter((data) => data.idUnidad == idunidadw?.value)
    const mappedOptions = unidadAreaFiltrado[0]?.controlAsistenciaAreas.map((est: any) => ({
      value: est?.idarea,
      label: est?.nombre
    }));

    setoptionArea(mappedOptions)
  }
