...
export const CEimagenesTablaMUI = ({ cuentaDatos }: any) => {
  ...
  const deleteImagenes = useCEDatosStore((state: any) => state.deleteImagenes);
    useEffect(() => {
        if (!cuentaDatos?.ordenesImagenes) return;
        setRows(cuentaDatos.ordenesImagenes);
    }, [cuentaDatos]);
    ...
    const handleEliminarMedicamento = async (medicamento: any) => {
        ...
        if (result.isConfirmed) {
            try {
               ...
               deleteImagenes(medicamento?.idproducto, medicamento?.puntoCarga)
  ...
  return (
    <div ref={containerRef} className="w-full">
            ...
            {isReady && cuentaDatos?.ordenesImagenes.length > 0 && (
                <Box sx={{ width: '100%' }}>
                    <DataGrid
                        rows={rows}
                        columns={columns}
