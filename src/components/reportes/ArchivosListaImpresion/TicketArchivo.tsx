...
const TicketArchivo = ({idcuentaatencion}:{idcuentaatencion:any}) => {
    const [Datos, setDatos] = useState<any>();
    const getDatosByCuenta = async (numcuenta: any) => {
        const { data } = await axios.get(`${process.env.apiurl}/TicketAdmision/${numcuenta}`)
        console.log(data)
        await setDatos(data)
    }
    useEffect(() => {
     getDatosByCuenta(idcuentaatencion);
    }, [idcuentaatencion])
...
window.print();
  ...
  return (
   <>
        ...
        {Datos && (
            <div className='container  print:block'>
