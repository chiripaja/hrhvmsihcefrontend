...
export const OrdenesRayosXTabla = ({ modificar = 0, datosEmergencia, recetaIdTemporal }:
    { modificar?: number, datosEmergencia: any, recetaIdTemporal: any }) => {
    const deleteOrdenesRayosX = useEmergenciaDatosStore((state: any) => state.deleteOrdenesRayosX);
    const handleDelete = async (indexToDelete: number) => {
        showConfirmDeleteAlert().then(async (result) => {
            if (result.isConfirmed) {
                ...
                deleteOrdenesRayosX(indexToDelete)
            }
            else {
    ...
    return (
        <div className="max-h-[300px] overflow-x-hidden overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
            ...
            <table className={datosEmergencia?.ordenesRayosX.length > 0 ? "tableT" : "hidden"} >
                <thead>
                    <tr>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-neutral-700">
                    {datosEmergencia?.ordenesRayosX.filter((data: any) => data.idrecetacabecera == recetaIdTemporal).map((data: any, index: any) => (
