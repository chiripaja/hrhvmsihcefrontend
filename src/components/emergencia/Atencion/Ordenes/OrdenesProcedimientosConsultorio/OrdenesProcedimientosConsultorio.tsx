...
import { OrdenesProcedimientosConsultorioTabla } from "./OrdenesProcedimientosConsultorioTabla";
import { useEmergenciaDatosStore } from "@/store/ui/emergenciadatos";

export const OrdenesProcedimientosConsultorio = ({ datosEmergencia, session }: any) => {
    ...
    const updateProcedimientosIdOrden = useEmergenciaDatosStore((state: any) => state.updateProcedimientosIdOrden);
    ...
    const FormImagenes: SubmitHandler<any> = async (data: any) => {
        const datosProcedimientos = {
            ...
            idTipoFinanciamiento: datosEmergencia?.idFormaPago,
    ...
    const handleCanastaProcedimientos = async () => {
        if (datosEmergencia?.recetaCabezeraProcedimientos[0]?.IdOrden !== null && datosEmergencia?.recetaCabezeraProcedimientos[0]?.IdOrden !== undefined) {
            const idorden = datosEmergencia?.recetaCabezeraProcedimientos[0].IdOrden;
            try {
            ...
            try {
                const datosCabezera = {
                    ...
                    idServicioPaciente: datosEmergencia?.idServicioEgreso ? datosEmergencia?.idServicioEgreso : datosEmergencia?.idServicio,
                    idTipoFinanciamiento: datosEmergencia?.idFormaPago,
                    idFuenteFinanciamiento: datosEmergencia?.idFuenteFinanciamiento,
                    idUsuario: session?.user?.id,
