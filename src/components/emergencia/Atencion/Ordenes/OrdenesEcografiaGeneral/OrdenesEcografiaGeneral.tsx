...
import { OrdenesEcografiaGeneralTablaRecetasCabecera } from './OrdenesEcografiaGeneralTablaRecetasCabecera';
import { OrdenesEcografiaGeneralTabla } from './OrdenesEcografiaGeneralTabla';

export const OrdenesEcografiaGeneral = ({ datosEmergencia, session }: any) => {
  ...
  const updateordenesEcografiaGeneral = useEmergenciaDatosStore((state: any) => state.updateordenesEcografiaGeneral);
  ...
  const fecthExamenesSelect = async () => {
      try {
          ...
          const response = await getData(`${process.env.apijimmynew}/FactCatalogoServicios/factaCatalogoServicioByIdPuntoCargaAndFormaPago/20/${datosEmergencia?.idFormaPago}`);
  ...
  const FormLaboratorio: SubmitHandler<any> = async (data: any) => {
      if (recetaIdTemporal == 0) {
          ...
          createordenesEcografiaGeneral(datosServicios)
  ...
  const updateReceta = async () => {
      try {
          if (recetaIdTemporal > 0) {
              const data = datosEmergencia?.ordenesEcografiaGeneral.filter((data: any) => data.idrecetacabecera == recetaIdTemporal);
  ...
  const crearNuevaReceta = async () => {
      ...
      try {
          ...
          const updatedOrdenes = await updateordenesEcografiaGeneral(
  ...
  return (
    ...
    <div className="bg-white rounded-md shadow-sm p-4">
        <h2 className="text-lg font-semibold text-gray-800 flex items-center justify-between relative">
            ...
            <span className="flex-grow">Ecografia General</span>
        ...
        <OrdenesEcografiaGeneralTablaRecetasCabecera datosEmergencia={datosEmergencia} handleOpenMenu={handleOpenMenu} />
    </div>
    ...
    >
        ...
        <div className="p-4">
            ...
            <OrdenesEcografiaGeneralTabla datosEmergencia={datosEmergencia} recetaIdTemporal={recetaIdTemporal} />
            <div className={datosEmergencia?.ordenesEcografiaGeneral.length > 0 ? "block" : "hidden"}>
