...
export const OrdenesFarmaciaTablaRecetasCabecera = ({ modificar = 0, datosEmergencia ,handleOpenMenu}: { modificar?: number, datosEmergencia: any,handleOpenMenu:any }) => {

    const deleteMedicamento = useCEDatosStore((state: any) => state.deleteMedicamento);
    const handleDelete = (indexToDelete: number) => {
        deleteMedicamento(indexToDelete)
    };
    const recetaCabeceraFarmacia = datosEmergencia?.recetaCabezera.filter(
    ...
    return (
        <div className="max-h-[300px] overflow-x-hidden overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
            <table className={recetaCabeceraFarmacia.length > 0 ? "tableT" : "hidden"} >
                <thead>
                    <tr>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-neutral-700">
                    {recetaCabeceraFarmacia.map((data: any, index: any) => (
                        <tr key={`${data.idReceta}-${data.FechaReceta}`}>
                            <td className="tabletd w-32">
                                ...
                                {data?.idEstado == "1" ?
                                    <div className="flex items-center gap-4">
                                        <Tooltip text="Eliminar">
                                            <button
                                             onClick={() => handleDelete(data?.idproducto)}
